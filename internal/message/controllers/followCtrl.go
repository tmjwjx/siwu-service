...
func FollowMessageCtrl(c *gin.Context) {
	...
	//进入业务层
	followList, err := logics.FollowMessageLogic(db, req, userId.(uint))
	if err != nil {
		globals.Log.Errorf("关注消息加载失败 err = %s", err)
	...
	// 返回响应
	data := response.NewAppData(globals.StatusOK, "关注消息加载成功", followList)
