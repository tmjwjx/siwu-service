...
// AddRole 添加角色
func (r *RoleReqContext) AddRole(role requests.Role) error {
...
// DeleteRole 删除角色
func (r *RoleReqContext) DeleteRole(ids requests.RoleIds) error {
	var num int64
...
// SearchRole 检索数据
func (r *RoleReqContext) SearchRole(searchRole requests.SearchRole) ([]*models.Role, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("RoleReqContext.SearchRole() -> %v", err)
	}
...
// UpdateRole 更新角色
func (r *RoleReqContext) UpdateRole(role requests.Role) error {
	// 判断该角色id是否存在
...
func (r *RoleReqContext) DispatchRole(dispatchRole requests.DispatchRole) error {
	// 判断数据是否正确
	// 判断userId是否存在
	user := repositories.QueryUserById(r.DB, dispatchRole.UserId)
	if user == nil {
		return fmt.Errorf("RoleReqContext.DispatchRole() err = 没有查询到id为 %v 的用户", dispatchRole.UserId)
	}

	userRoleSli := make([]*models.UserRole, 0)
	userId := dispatchRole.UserId
	for _, id := range dispatchRole.Ids {
		// 判断角色Id是否存在
		role := repositories.QueryRoleById(r.DB, id)
		if role == nil {
			return fmt.Errorf("RoleReqContext.DispatchRole err = 没有查询到id为 %v 的角色", id)
		}

		// 添加的切片里
		userRoleSli = append(userRoleSli, &models.UserRole{
			UserId: userId,
			RoleId: id,
		})
	}
	...
	// 插入数据
	err := repositories.InsertObjects(r.DB, userRoleSli)
	if err != nil {
		return fmt.Errorf("RoleReqContext.DispatchRole() -> %v", err)
	}
	return nil
