...
func BsLogin(c *gin.Context) {
	...
	if !internalUtils.IsValidPassword(bsLogicMsg.Password) {
		response.Failed(c, http.StatusBadRequest, response.NewAppErr(globals.StatusBadRequest, fmt.Errorf("BsLogin() : 密码必须要同时包含字母、数字、特殊字符，长度在8到16位之间"), nil))
		return
	}
...
// BsLogout
// @Description: 后台登出
// @Author lizhuang 2024-10-21 21:21:20
// @param        c *gin.Context
func BsLogout(c *gin.Context) {
	_, exists := c.Get("id")
	if !exists {
		response.Failed(c, http.StatusUnauthorized, response.NewAppErr(globals.StatusUnauthorized, fmt.Errorf("BsLogout() err = 无法获取 id"), nil))
		return
	}
